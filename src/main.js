...
import { getLastRefinement, invalidateCustomModel } from './glasses/loadGlassesModel.js';
...
const glbRefine = document.getElementById('glb-refine');
...
glbInput.addEventListener('change', async () => {
  ...
  try {
    ...
    // A model generated by an image-to-3D tool arrives with solid lenses,
    // whether it came through the service or was carried in by hand. Same
    // problem, same fix — so the refinement is offered for a plain upload too,
    // not only for models this app generated itself.
    await saveCustomModel(glb, {
      source: 'upload',
      name: file.name,
      frameWidthMM: 140,
      refine: glbRefine.checked,
    });
    ...
    const size = `${mm(info.sizeCM.width)}×${mm(info.sizeCM.height)}×${mm(info.sizeCM.depth)}`;

    // The refinement runs while the style is being switched to, so give it a
    // moment before reporting — an empty result here would read as a failure.
    setTimeout(() => {
      const refined = getLastRefinement();
      let lensLine = '';
      if (glbRefine.checked) {
        lensLine = refined?.lensFound
          ? `<br>✔ عدسی پیدا و شفاف شد (${refined.lensFaces} مثلث).`
          : `<br>⚠ عدسی پیدا نشد${refined?.reason ? ` (${refined.reason})` : ''} — مدل با عدسی توپر نمایش داده می‌شود.`;
      }
      showGlbStatus(
        `مدل «${file.name}» بارگذاری شد — تراز خودکار شد به ${size} میلی‌متر.` +
          lensLine +
          '<br>اگر جای عینک روی صورت دقیق نبود، از «تنظیم دقیق» استفاده کنید.'
      );
    }, 2500);
    showGlbStatus(`مدل «${file.name}» بارگذاری شد — در حال تحلیل عدسی‌ها…`);
  } catch (err) {
    console.error(err);
