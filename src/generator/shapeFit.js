...
/**
 * Height profile of a temple arm, sampled along its length from the side
 * photo's mask. Measuring the arm at stations — rather than extruding its
 * traced outline — is what stops a nick in the silhouette becoming a spike
 * in the model, and gives the taper a real frame has.
 *
 * @returns {{lengthPx:number, baseHeightPx:number, stations:number[], bendAt:number}}
 *   stations are heights normalised to baseHeightPx (the height at the hinge);
 *   bendAt is where the arm starts to turn down over the ear, as a fraction of
 *   length.
 */
export function measureTempleProfile(mask, width, height, comp, stations = 24) {
  const x0 = comp.minX;
  const x1 = comp.maxX;
